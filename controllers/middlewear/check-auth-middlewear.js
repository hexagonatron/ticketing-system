...
module.exports = {
    authenticateUser(req, res, next) {
        ...
        try {
            ...
            if (err.message === "jwt expired") return res.status(400).json({ error: "Expired token" });
    ...
    checkAdmin(req, res, next) {
        ...
        if (req.user === undefined) return res.status(400).json({ error: "Authentication error" });

        //if authed user isn't an admin then don't give them access
        if (req.user.role !== 'admin') return res.status(400).json({ error: "User not permitted" });
